...
void Hooks::HKDrawSetColor(Color col) {

	if (g_cl.m_console_open && g_menu.main.misc.custom_console.get())
	{
		// New colors
		Color color = g_menu.main.misc.console_color.get();
		color.a() = g_menu.main.misc.console_color_alpha.get();

		g_hooks.m_surface.GetOldMethod< DrawSetColor_t >(ISurface::DRAWSETCOLOR)(this, color);
		return;
	}

	g_hooks.m_surface.GetOldMethod< DrawSetColor_t >(ISurface::DRAWSETCOLOR)(this, col);
}
