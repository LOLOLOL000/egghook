...
template <class T>
__forceinline T Lerp2(float flPercent, T const& A, T const& B)
{
...
void events::round_start( IGameEvent* evt ) { 
	...
	for( int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i ) {
		...
		g_visuals.m_dormant_data[i].m_alpha = Lerp2(g_visuals.m_dormant_data[i].m_alpha, 0.f, 8.f * g_csgo.m_globals->m_frametime);
		...
		if (data->best_point.filled)
			data->best_point.reset();
	}
...
void events::player_hurt( IGameEvent* evt ) {
    ...
    if( evt ) {
        ...
        if( attacker->index() > 0 && attacker->index() < 64 && victim->index() == g_csgo.m_engine->GetLocalPlayer())
	...
	if (evt)
	{
		...
		for (iter = g_visuals.impacts.begin(); iter != g_visuals.impacts.end(); ) {
			if (time > iter->time + 25) {
				iter = g_visuals.impacts.erase(iter);
				continue;
			}
		...
		info.alpha = 255;
		info.player = victim;
		info.damage = evt->m_keys->FindKey(HASH("dmg_health"))->GetInt();
...
void events::bullet_impact( IGameEvent* evt ) {
	...
	if (g_cl.m_processing)
	{
		...
		// forward event to resolver impact processing.
		static auto sv_showimpacts_time = g_csgo.m_cvar->FindVar(HASH("sv_showimpacts_time"));
		...
		Color color = g_menu.main.misc.server_impact_color.get();
		color.a() = (g_menu.main.misc.impact_alpha.get() * 2.55);

		if (g_menu.main.misc.server_impacts.get())
			g_csgo.m_debug_overlay->AddBoxOverlay(vec3_t(info.x, info.y, info.z), vec3_t(-2, -2, -2), vec3_t(2, 2, 2), ang_t(0, 0, 0), color.r(), color.g(), color.b(), color.a(), sv_showimpacts_time->GetFloat());
	}
}
