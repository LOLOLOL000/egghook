...
vec3_t HVH::get_predicted_pos()
{
	...
	for (int i = 0; i < max_predict_ticks; ++i)
	{
		...
		vec3_t local_origin = eye_position + pred_velocity;
		int flags = g_cl.m_local->m_fFlags();

		g_movement.extrapolate(g_cl.m_local, local_origin, pred_velocity, flags, flags & FL_ONGROUND);
...
void HVH::AutoDirection() {
	...
	{
		vec3_t head_position(38.f * cos(n) + eye_position.x, 38.f * sin(n) + eye_position.y, eye_position.z);

		Ray ray(eye_position, head_position);
		CGameTrace tr;
		CTraceFilterSimple trace;
		trace.m_pass_ent1 = g_cl.m_local;

		g_csgo.m_engine_trace->TraceRay(ray, 0x4600400B, &trace, &tr);

		if (tr.m_fraction != 1.f)
		{
			if (auto diff = math::AngleDiff(math::rad_to_deg(n), g_cl.m_view_angles.y); abs(diff) <= 120.f)
			{
				if (diff > 0) {
					right++;
	...
	if (abs(left - right) > 1)
	{
		m_facing_left = left < right;
		m_facing_right = !m_facing_left;
		m_facing_back = false;
		m_facing_forward = false;

		m_auto = math::NormalizedAngle(m_view - (left < right ? -90.f : 90.f));
		return;
	}
...
void HVH::GetAntiAimDirection() {
	...
	if (state->m_flVelocityLengthXY != 0.f && state->m_bOnGround)
		this->m_lastmove = g_cl.m_cmd->m_view_angles.y;

	if (state->m_bOnGround && state->m_flVelocityLengthXY < 0.1f && g_menu.main.antiaim.avoid_lastmove.get())
	{
		float delta = std::abs(math::AngleDiff(this->m_lastmove, g_cl.m_cmd->m_view_angles.y));

		if (delta < 35.f)
		{
			if (m_facing_left)
				m_direction -= -g_menu.main.antiaim.min_amount.get();
			else if (m_facing_right)
				m_direction += g_menu.main.antiaim.min_amount.get();
		}
	...
	if (state->m_bLeftTheGroundThisFrame && g_menu.main.antiaim.avoid_lastmove.get())
		m_direction += g_menu.main.antiaim.min_amount.get();
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		bool stand = g_menu.main.antiaim.body_fake_stand.get() > 0 || g_menu.main.antiaim.flick_twice.get() && m_mode == AntiAimMode::STAND;
		...
		if (stand && !g_cl.m_lag && g_csgo.m_globals->m_curtime >= (g_cl.m_body_pred - g_cl.m_anim_frame) && g_csgo.m_globals->m_curtime < g_cl.m_body_pred) {
			...
			if (g_menu.main.antiaim.flick_twice.get())
				g_cl.m_cmd->m_view_angles.y -= 36.f;
		}
...
void HVH::OnPredictEnd()
{
	auto& correct = choked_ticks.emplace_front();

	correct.cmd = g_cl.m_cmd->m_command_number;
	correct.choke = g_csgo.m_cl->choked_commands + 1;
	correct.tickcount = g_csgo.m_globals->m_tick_count;
	...
	while (choked_ticks.size() > (int)(2.f / g_csgo.m_globals->m_interval))
		choked_ticks.pop_back();

	auto& out = commands.emplace_back();
	out.outgoing = *g_cl.m_packet;
	out.used = false;
	out.cmd = g_cl.m_cmd->m_command_number;
	out.prev_cmd = 0;
	while (commands.size() > g_cl.m_rate)
		commands.pop_front();
...
void HVH::bypass_choke_limit()
{
	static int old_choke = 0;

	static auto addr = pattern::find(g_csgo.m_engine_dll, XOR("B8 ? ? ? ? 3B F0 0F 4F F0 89 5D FC"));

	if (old_choke != g_cl.m_max_lag)
	{
		auto address = addr.add(1).as< uint8_t* >();

		uint32_t choke_clamp = g_cl.m_max_lag + 3;
...
void HVH::SendPacket() {
	...
	int choke = g_csgo.m_cl->choked_commands;
	int choke_amount = (int)g_menu.main.antiaim.lag_limit.get();

	if (choke_amount == 0 || g_csgo.m_gamerules->m_bFreezePeriod() || g_cl.m_flags & FL_FROZEN)
	{
		*g_cl.m_packet = true;
		return;
	}
	...
	// dont lag while firing
	if (g_cl.is_firing() && !g_input.GetKeyState(g_menu.main.movement.fakewalk.get()))
		*g_cl.m_packet = true;

	// too much choked commands
	if (choke > 0 && std::abs(g_csgo.m_globals->m_tick_count - g_cl.sent_tick_count) > choke_amount || choke >= choke_amount)
	{
		// disable lag
		*g_cl.m_packet = true;
	}
