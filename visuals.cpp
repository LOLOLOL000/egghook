...
InfernoData::InfernoData(Entity* inferno) noexcept
{
	...
	points.reserve(inferno->m_fireCount());
	for (int i = 0; i < inferno->m_fireCount(); ++i) {
		if (inferno->m_bFireIsBurning()[i])
			points.emplace_back(inferno->m_fireXDelta()[i] + origin.x, inferno->m_fireYDelta()[i] + origin.y, inferno->m_fireZDelta()[i] + origin.z);
	}
}
...
void Visuals::Hitmarker3D()
{
	...
	int m_actual_size = hitmarkers.size() - 1;

	for (iter = hitmarkers.begin(); iter != hitmarkers.end();)
	{
		bool expired = time > iter->impact.time + 1200;
		...
		auto m_end = hitmarkers[m_actual_size];
		...
		const auto color = iter->damage >= iter->player->m_iHealth() ? Color(255, 0, 0, iter->alpha) : Color(255, 255, 255, iter->alpha);
...
void Visuals::DrawPlayer(Player* player) {
	...
	bool bone_esp = enemy;
	if (bone_esp && g_menu.main.players.skeleton.get(0))
		DrawSkeleton(player, alpha);

	if (bone_esp && g_menu.main.players.skeleton.get(1))
	...
	if (box_esp) {
		Color col = g_menu.main.players.box_enemy.get();
		col.a() = 210;
		render::rect(box.x, box.y, box.w, box.h, col);
	...
	if (name_esp) {
		...
		std::string name{ std::string(info.m_name) };
		if (name.length() > 15) {
			name.resize(15);
			name.append(XOR("..."));
		}
	...
	if (health_esp) {
		const int bar_size = std::clamp(int((player->m_iHealth() * box.h) / 100), 0, box.h);
		...
		auto color = Color(r, g, 0, alpha);

		render::rect_filled(box.x - 6, box.y - 1, 4, box.h + 2, Color(0, 0, 0, low_alpha));
		render::rect_filled(box.x - 5, box.y + (box.h - bar_size), 2, bar_size, color);

		// draw health amount when it's lethal damage
		if (player->m_iHealth() <= 92 || player->m_iHealth() > 100) {
			// note - michal;
			...
			render::esp_small.string(box.x - 5, box.y + (box.h - bar_size) - 7, Color(255, 255, 255, low_alpha), std::to_string(player->m_iHealth()), render::ALIGN_CENTER);
		}
	}
	...
	// draw flags.
	{
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			// hit.
			if (*it == 6 && g_aimbot.m_target && g_aimbot.m_target == player)
				flags.push_back({ XOR("HIT"), { 255, 255, 255, low_alpha} });
		}
			...
			if (weapon) {
				...
				if (data) {
					...
					if (max != -1 && g_menu.main.players.ammo.get()) {
						...
						const int nCurrentClip = player->dormant() ? data->m_max_clip1 : weapon->m_iClip1();

						// also prevent division by zero, lol
						float flBoxMultiplier = (float)nCurrentClip / data->m_max_clip1;

						bool bReloading = false;
						auto pReloadLayer = player->m_AnimOverlay()[1];
						if (pReloadLayer.m_owner) {
							const int nActivity = player->GetSequenceActivity(pReloadLayer.m_sequence);

							if (nActivity == 967 && pReloadLayer.m_weight != 0.f) {
								// smooth out the ammo bar for reloading players
								flBoxMultiplier = pReloadLayer.m_cycle;
								bReloading = true;
							}
						}
						...
						render::rect_filled(box.x, box.y + box.h + offset, box_width, 2, clr);

						// ammo is less than 90% of the max ammo
						if (nCurrentClip > 0 && nCurrentClip <= int(std::floor(float(data->m_max_clip1) * 0.9f)) && !bReloading) {
							render::esp_small.string(box.x + box_width, (box.y + box.h + offset) - 3, Color(255, 255, 255, low_alpha),
								std::to_string(nCurrentClip), render::ALIGN_CENTER);
						}
					...
					/*if (g_menu.main.players.shot_time.get()) {
						const int m_iRenderWidth = box.w * 2 - 2;

						const float m_flShotCooldown = data->m_cycletime;
						const float m_flNextShotTime = weapon->m_flNextPrimaryAttack();

						float m_flFraction = (m_flNextShotTime - g_csgo.m_globals->m_curtime) / m_flShotCooldown;
						m_flFraction = std::clamp<float>(m_flFraction, 0.f, 1.0f);

						const float m_flWidth = m_iRenderWidth * (1.0f - m_flFraction);

						Color clr = g_menu.main.players.shot_time_color.get();
						clr.a() = alpha;

						// draw.
						render::rect_filled(box.x, box.y + box.h + 2 + offset, box.w, 4, { 10, 10, 10, low_alpha });
						render::rect(box.x + 1, box.y + box.h + 3 + offset, m_flWidth, 2, clr);
...
void Visuals::DrawHistorySkeleton(Player* player, int opacity) {
	...
	BoneArray out[128];
	if (g_chams.GenerateLerpedMatrix(player->index(), out))
	{
		for (int i{ }; i < hdr->m_num_bones; ++i)
		{
			...
			out->get_bone(bone_pos, i);
			out->get_bone(parent_pos, parent);

			Color clr = g_menu.main.players.skeleton_enemy.get();
			clr.a() = g_menu.main.players.skeleton_alpha.get() / 255.f;
...
void Visuals::RenderGlow() {
	...
	float blend_proj = g_menu.main.players.glow_projectile_blend.get() / 100.f;
	...
	for (int i{ }; i < g_csgo.m_glow->m_object_definitions.Count(); ++i) {
		...
		if (obj->m_pEntity->IsPlayer())
		{
			if (!g_menu.main.players.glow.get(0))
				continue;
			...
			obj->m_nGlowStyle = g_menu.main.players.glow_type.get();
			obj->m_bFullBloomRender = g_menu.main.players.glow_bloom.get();
			obj->m_flRed = (float)g_menu.main.players.glow_enemy.get().r() / 255.f;
			obj->m_flGreen = (float)g_menu.main.players.glow_enemy.get().g() / 255.f;
			obj->m_flBlue = (float)g_menu.main.players.glow_enemy.get().b() / 255.f;
			obj->m_flAlpha = opacity * blend;
			obj->m_bRenderWhenOccluded = true;
			obj->m_bRenderWhenUnoccluded = false;
		}
		else
		{
			if (obj->m_pEntity->is(HASH("CInferno")) ||
				obj->m_pEntity->is(HASH("CSmokeGrenadeProjectile")) ||
				obj->m_pEntity->is(HASH("CBaseCSGrenadeProjectile")) ||
				obj->m_pEntity->is(HASH("CMolotovProjectile")) ||
				obj->m_pEntity->is(HASH("CDecoyProjectile")))
			{
				if (!g_menu.main.players.glow.get(1))
					continue;

				obj->m_nGlowStyle = g_menu.main.players.glow_type.get();
				obj->m_bFullBloomRender = g_menu.main.players.glow_bloom.get();
				obj->m_flRed = (float)g_menu.main.players.glow_projectile_color.get().r() / 255.f;
				obj->m_flGreen = (float)g_menu.main.players.glow_projectile_color.get().g() / 255.f;
				obj->m_flBlue = (float)g_menu.main.players.glow_projectile_color.get().b() / 255.f;
				obj->m_flAlpha = blend;
				obj->m_bRenderWhenOccluded = true;
				obj->m_bRenderWhenUnoccluded = false;
			}
...
void  Visuals::Update() {
	...
	if (lastFrame == g_csgo.m_globals->m_frame)
		return;

	lastFrame = g_csgo.m_globals->m_frame;

	infernoData.clear();

	for (int i = 1; i <= g_csgo.m_entlist->GetHighestEntityIndex(); ++i)
	{
		const auto entity = g_csgo.m_entlist->GetClientEntity(i);
		if (!entity)
			continue;

		if (!entity->IsPlayer())
		{
			if (entity->dormant() || entity->IsBaseCombatWeapon())
				continue;

			if (entity->is(HASH("CInferno")))
			{
...
void Visuals::ImpactData()
{
	...
	if (!g_menu.main.misc.client_impacts.get()) return;
	...
	Color color = g_menu.main.misc.client_impact_color.get();
...
void Visuals::drawMolotovHull()
{
	static const auto flameCircumference = [] {
		...
		for (std::size_t i = 0; i < points.size(); ++i) {
			...
			points[i] = vec3_t{ flameRadius * std::cos(math::deg_to_rad(i * (360.0f / points.size()))),
								flameRadius * std::sin(math::deg_to_rad(i * (360.0f / points.size()))),
								0.0f };
		}
		return points;
		}();

		for (const auto& molotov : infernoData)
		{
			for (const auto& pos : molotov.points)
			{
				std::array<vec2_t, flameCircumference.size()> screenPoints;
				std::size_t count = 0;

				for (const auto& point : flameCircumference) {
					if (render::WorldToScreen(pos + point, screenPoints[count]))
						++count;
				}
				...
				constexpr auto orientation = [](const vec2_t& a, const vec2_t& b, const vec2_t& c) {
