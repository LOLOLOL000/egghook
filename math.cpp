...
vec3_t math::vector_rotate(const vec3_t& in1, const ang_t& in2)
{
	...
	return math::vector_rotate(in1, mat);
}
...
float math::smooth_step_bounds(float edge0, float edge1, float x)
{
	float bounds = std::clamp((x - edge0) / (edge1 - edge0), 0.f, 1.f);
	return bounds * bounds * (3 - 2 * bounds);
}
...
void math::angle_to_vectors(vec3_t angles, vec3_t& forward, vec3_t& right, vec3_t& up)
{
	float angle;
...
ang_t math::angle_from_vectors(vec3_t a, vec3_t b)
{
	ang_t angles{ };
...
void math::ClampAngles(ang_t& angAngles)
{
...
void vec3_tAngles3(const vec3_t& forward, vec3_t& up, ang_t& angles)
{
...
bool math::IntersectRayWithAABB(vec3_t& origin, vec3_t& dir, vec3_t& min, vec3_t& max)
{
...
void vec3_tRotate(const vec3_t& in1, const matrix3x4_t& in2, vec3_t& out)
{
...
ang_t math::normalize_ang(ang_t ang, bool fix_pitch)
{
	while (ang.y < -180.0f)
		ang.y += 360.0f;
	while (ang.y > 180.0f)
...
__forceinline vec3_t vec_angle(vec3_t vec) {
	...
	if (vec.x == 0.0f && vec.y == 0.0f) {
		...
		ret.x = math::rad_to_deg(std::atan2f(-vec.z, vec.length_2d()));
		ret.y = math::rad_to_deg(std::atan2f(vec.y, vec.x));
...
vec3_t math::CalcAngle(const vec3_t& vecSource, const vec3_t& vecDestination) {
	auto ret = vec_angle(vecDestination - vecSource);
...
void vec3_tAngles1(const vec3_t& vecForward, vec3_t& vecAngles)
{
	vec3_t vecView;
...
ang_t vec3_tAngles(const vec3_t& vecForward)
{
	float flPitch, flYaw;
	...
	{
		flPitch = std::atan2f(-vecForward.z, vecForward.length_2d()) * 180.f / M_PI;
	...
	return ang_t(flPitch, flYaw, 0.f);
}
...
void math::VectorAngles(const vec3_t& forward, ang_t& angles, vec3_t* up) {
	...
	if (up && len > 0.001f) {
		pitch = math::rad_to_deg(std::atan2(-forward.z, len));
...
void math::AngleVectors(const vec3_t& angView, vec3_t* pForward, vec3_t* pRight, vec3_t* pUp)
{
	float sp, sy, sr, cp, cy, cr;

	ScalarSinCos(&sp, &cp, deg_to_rad(angView.x));
	ScalarSinCos(&sy, &cy, deg_to_rad(angView.y));
	ScalarSinCos(&sr, &cr, deg_to_rad(angView.z));
...
vec3_t math::VectorTransform(const vec3_t& transform, const matrix3x4_t& matrix) {
	vec3_t out = {
