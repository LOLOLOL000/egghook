...
namespace math {
	...
	float smooth_step_bounds(float edge0, float edge1, float x);

	void angle_to_vectors(vec3_t angles, vec3_t& forward);
	void angle_to_vectors(vec3_t angles, vec3_t& forward, vec3_t& right, vec3_t& up);
	ang_t angle_from_vectors(vec3_t a, vec3_t b);
	...
	void ClampAngles(ang_t& angAngles);
	...
	bool IntersectRayWithAABB(vec3_t& origin, vec3_t& dir, vec3_t& min, vec3_t& max);
	void VectorRotate(const vec3_t& in1, const matrix3x4_t& in2, vec3_t& out);
	vec3_t extrapolate_position(vec3_t pos, vec3_t extension, int amount, float interval);

	ang_t normalize_ang(ang_t ang, bool fix_pitch);
	...
	vec3_t VectorTransform(const vec3_t& transform, const matrix3x4_t& matrix);
	ang_t VectorAngles(const vec3_t& vecForward);
	...
	void	AngleVectors(const vec3_t& angView, vec3_t* pForward, vec3_t* pRight = nullptr, vec3_t* pUp = nullptr);
  ...
  // float Lerp( float flPercent, float A, float B );
	...
	template <class T>
	T naive_lerp(float a, float b, float t)
	{
		return a + t * (b - a);
	}
	...
	template < typename t >
	__forceinline void clamp(t& ang) {
