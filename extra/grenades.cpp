...
int Grenades::add_glow_box(vec3_t origin, ang_t ang_orientation, vec3_t mins, vec3_t maxs, Color clr, float lifetime)
{
	using add_glow_box_t = int(__thiscall*)(void*, vec3_t, ang_t, vec3_t, vec3_t, Color, float);
	return g_csgo.m_add_glow_box.as<add_glow_box_t>()(g_csgo.m_glow, origin, ang_orientation, mins, maxs, clr, lifetime);
}
