...
void Chams::SetupMaterial( int mat, Color col, bool z_flag ) {
	...
	material->set_material_var_flag( MATERIAL_VAR_ZNEARER, z_flag );
	material->set_material_var_flag( MATERIAL_VAR_NOFOG, z_flag );
	material->set_material_var_flag( MATERIAL_VAR_IGNOREZ, z_flag );
...
void Chams::init( ) {
	...
	metallic = g_csgo.m_material_system->FindMaterial("simple_ignorez_reflective", XOR("Model textures"));
	metallic->increment_reference_count();

	pred_glass = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/cologne_prediction/cologne_prediction_glass"), XOR("Other textures"));
	pred_glass->increment_reference_count();

	crystal = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/trophy_majors/crystal_clear"), XOR("Other textures"));
	crystal->increment_reference_count();

	velvet = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/trophy_majors/velvet"), XOR("Other textures"));
	velvet->increment_reference_count();

	gold = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/wildfire_gold/wildfire_gold_detail"), XOR("Other textures"));
...
void Chams::RenderHistoryChams(int index) {
	...
	if (enemy) {
		...
		SetupMaterial(g_menu.main.players.chams_history_mat.get(), g_menu.main.players.chams_enemy_history_col.get(), true);
...
void Chams::RenderPlayer(Player* player) {
	...
	if (player->m_bIsLocalPlayer()) {
		if (g_menu.main.players.chams_local_scope.get() && player->m_bIsScoped())
			...
			// set material and color.
			SetupMaterial(g_menu.main.players.chams_local_mat.get(), g_menu.main.players.chams_local_col.get(), false);
		}
	...
	if (enemy && g_menu.main.players.chams_enemy.get(0)) {
		if (g_menu.main.players.chams_enemy.get(1)) {
			...
			SetupMaterial(g_menu.main.players.chams_enemy_mat.get(), g_menu.main.players.chams_enemy_invis.get(), true);

			player->DrawModel();
		}

		SetAlpha(g_menu.main.players.chams_enemy_blend.get() / 100.f);
		SetupMaterial(g_menu.main.players.chams_enemy_mat.get(), g_menu.main.players.chams_enemy_vis.get(), false);
