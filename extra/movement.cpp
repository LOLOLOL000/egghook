...
void Movement::update_ground_ticks()
{
	if (g_cl.m_local->m_fFlags() & FL_ONGROUND)
	{
		if (this->m_ground_ticks <= 3)
			this->m_ground_ticks++;
	}
	else
	{
...
bool Movement::on_ground()
{
	return m_ground_ticks >= 3;
}

void Movement::ViewModelChanger()
{
	if (!g_cl.m_local || !g_cl.m_local->alive())
		return;

	static auto backup_viewmodel_x = g_csgo.viewmodel_offset_x->GetFloat();
	static auto backup_viewmodel_y = g_csgo.viewmodel_offset_y->GetFloat();
	static auto backup_viewmodel_z = g_csgo.viewmodel_offset_z->GetFloat();

	if (g_menu.main.misc.custom_viewmodel.get()) {
		g_csgo.viewmodel_offset_x->SetValue(g_menu.main.misc.viewmodel_x.get());
		g_csgo.viewmodel_offset_y->SetValue(g_menu.main.misc.viewmodel_y.get());
		g_csgo.viewmodel_offset_z->SetValue(g_menu.main.misc.viewmodel_z.get());
	}
	else {
		g_csgo.viewmodel_offset_x->SetValue(backup_viewmodel_x);
		g_csgo.viewmodel_offset_y->SetValue(backup_viewmodel_y);
		g_csgo.viewmodel_offset_z->SetValue(backup_viewmodel_z);
	}
}

void Movement::force_speed(float max_speed)
{
	// as did in game movement of CSGO:
  // reference: CCSGameMovement::CheckParameters

	if (g_cl.m_local->m_MoveType() == MOVETYPE_NOCLIP || g_cl.m_local->m_MoveType() == MOVETYPE_ISOMETRIC || g_cl.m_local->m_MoveType() == MOVETYPE_OBSERVER)
		return;

	float sidemove = g_cl.m_cmd->m_side_move;
	float forwardmove = g_cl.m_cmd->m_forward_move;
	float upmove = g_cl.m_cmd->m_up_move;
	...
	g_cl.m_cmd->m_forward_move *= kys;
	g_cl.m_cmd->m_side_move *= kys;
	g_cl.m_cmd->m_up_move *= kys;
}
...
void Movement::extrapolate(Player* player, vec3_t& origin, vec3_t& velocity, int& flags, bool on_ground)
{
	if (!(flags & FL_ONGROUND))
		velocity.z -= game::TICKS_TO_TIME(g_csgo.sv_gravity->GetFloat());
	else if (player->m_fFlags() & FL_ONGROUND && !on_ground)
		velocity.z = g_csgo.sv_jump_impulse->GetFloat();
	...
	g_csgo.m_engine_trace->TraceRay(Ray(src, end, player->m_vecMins(), player->m_vecMaxs()), MASK_SOLID, (ITraceFilter*) & filter, &t);
	...
	if (t.m_fraction != 1.f)
	{
		for (auto i = 0; i < 2; i++)
		{
			...
			g_csgo.m_engine_trace->TraceRay(Ray(t.m_endpos, end, player->m_vecMins(), player->m_vecMaxs()), MASK_SOLID, (ITraceFilter*)&filter, &t);
	...
	g_csgo.m_engine_trace->TraceRay(Ray(origin, end, player->m_vecMins(), player->m_vecMaxs()), MASK_SOLID, (ITraceFilter*)&filter, &t);
...
void Movement::FakeWalk( ) {
	...
	if( ticks > ( ( max - 1 ) - g_csgo.m_cl->choked_commands ) || !g_csgo.m_cl->choked_commands) {
