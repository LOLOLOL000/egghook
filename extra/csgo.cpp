...
bool CSGO::init( ) {
	...
	model_cache			 = interfaces.get< c_mdl_cache*>(HASH("MDLCache"));
	...
	// classes by sig.
	//device = **pattern::find(m_shaderapidx9_dll, XOR("A1 ? ? ? ? 50 8B 08 FF 51 0C")).add(0x1).as< IDirect3DDevice9***>();
	...
	m_add_glow_box = pattern::find(m_client_dll, XOR("55 8B EC 53 56 8D 59"));
	...
	m_NextTick = pattern::find(m_client_dll, XOR("55 8B EC 56 57 8B F9 8B B7 E8"));
	...
	m_nCachedBonesPosition = *(int*)(g_csgo.cachebonedata + 0x2) + 0x4;
	m_nCachedBonesRotation = *(int*)(g_csgo.cachebonedata + 0x25) + 0x4;
	attachments_helper				= pattern::find(m_client_dll, XOR("55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4"));
	...
	viewmodel_offset_x->m_callbacks.m_Size = 0;
	viewmodel_offset_y->m_callbacks.m_Size = 0;
	viewmodel_offset_z->m_callbacks.m_Size = 0;
