...
public:
	...
	int m_body_index;
	float m_body_flick;
	float m_flick_angle_missed;
	int m_air_flick_miss;
	int m_no_flick_index;
	int m_lby_index;
...
public:
	...
	std::vector< std::pair< int, int > > GetHitboxes( LagRecord* record );
	std::vector< std::pair< vec3_t, bool > > SetupHitboxPoints( LagRecord* record, BoneArray* bones, int index );
...
public:
	...
	void init( );
	bool AutoStop();
	bool is_accuracy_valid(Player* player, const ang_t& angle);
	void StripAttack( );
	void think( );
