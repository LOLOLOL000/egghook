...
#define HOOK( target, hook, original ) \
    if( MH_CreateHook( Address{ target }.as< LPVOID > ( ), \
...
#define HOOK_VFUNC( vft, index, hook, original ) \
    if( MH_CreateHook( ( *Address{ vft }.as< LPVOID** > ( ) )[ index ], \
        reinterpret_cast< LPVOID > ( &hook ), reinterpret_cast< LPVOID* > ( &original ) ) != MH_OK ) \
        THROW_IF_DBG( "can't hook " #hook "." ) \
...
namespace game {
	...
	__forceinline float GetLerp() {
		float total_lerp = 0.f;

		float update_rate_value = g_csgo.cl_updaterate->GetFloat();

		if (g_csgo.m_cvar->FindVar(HASH("sv_minupdaterate")) && g_csgo.m_cvar->FindVar(HASH("sv_maxupdaterate")))
			update_rate_value = std::clamp< float >(update_rate_value, g_csgo.m_cvar->FindVar(HASH("sv_minupdaterate"))->GetFloat(), g_csgo.m_cvar->FindVar(HASH("sv_maxupdaterate"))->GetFloat());

		bool use_interp = g_csgo.m_cvar->FindVar(HASH("cl_interpolate"))->GetInt();
		if (use_interp)
		{
			float lerp_ratio = g_csgo.m_cvar->FindVar(HASH("cl_interp_ratio"))->GetFloat();
			if (lerp_ratio == 0.f)
				lerp_ratio = 1.f;

			float lerp_amount = g_csgo.m_cvar->FindVar(HASH("cl_interp"))->GetFloat();

			if (g_csgo.m_cvar->FindVar(HASH("sv_client_min_interp_ratio")) && g_csgo.m_cvar->FindVar(HASH("sv_client_max_interp_ratio"))
				&& g_csgo.m_cvar->FindVar(HASH("sv_client_min_interp_ratio"))->GetFloat() != -1)
			{
				lerp_ratio = std::clamp< float >(lerp_ratio, g_csgo.m_cvar->FindVar(HASH("sv_client_min_interp_ratio"))->GetFloat(),
					g_csgo.m_cvar->FindVar(HASH("sv_client_max_interp_ratio"))->GetFloat());
			}
			else
			{
				if (lerp_ratio == 0.f)
