...
public:
	...
	{
		return rgba
			? ((m_r & 0xFF) << 24) + ((m_g & 0xFF) << 16) + ((m_b & 0xFF) << 8) + (m_a & 0xFF)
			: ((m_a & 0xFF) << 24) + ((m_b & 0xFF) << 16) + ((m_g & 0xFF) << 8) + (m_r & 0xFF);
	}
	...
	__forceinline Color alpha(int value) const { return Color(m_r, m_g, m_b, value); }
