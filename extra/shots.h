...
public:
	__forceinline ShotRecord(
		...
		m_record{ m_record ? m_record : nullptr }, m_hitbox{ hitbox } {}
...
public:
	...
	__forceinline void add(
		const vec3_t& src, Player* target, const int next_shift_amount, const std::ptrdiff_t cmd_num, const float sent_time, const float latency, LagRecord* m_record, int hitbox
	) {
		m_shots.emplace_back(src, target, next_shift_amount, cmd_num, sent_time, latency, m_record, hitbox);
