	...
	unsigned int find_in_datamap(datamap_t* map, const char* name)
	{
		while (map)
		{
			for (auto i = 0; i < map->m_size; ++i)
			{
				...
				if (!strcmp(name, map->m_desc[i].m_name))
					return map->m_desc[i].m_offset[0];

				if (map->m_desc[i].m_type == 10)
				{
					if (map->m_desc[i].m_td)
					{
						unsigned int offset;

						if (offset = find_in_datamap(map->m_desc[i].m_td, name))
							return offset;
					}
				}
			}
